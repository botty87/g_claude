...
> Stato: **Fase 0→5 FATTE e verificate**. Migrazione completa e testata. Branch `feature/claude-sdk` pronto per merge (gestire prima i file `macos/*` WIP pre-esistenti). **Uso interno: niente Developer ID/notarization**. Remoto/WS/Tailscale = fuori scope (fase futura).
>
> **Fase 5 (test/docs)**: backend TS — `node:test` per `mapping.ts` (SDKMessage→evento) e `permissions.ts` (`decisionFor`), 14 test. Dart — `sidecar_client_datasource_test.dart` (36 test: mapping protocollo↔ClaudeEvent con fake transport, sid-filter, completion, payload REQ, cache questions) + `claude_sessions_cubit_events_test.dart` (8 bloc_test: sessionInit/planProposed/permissionRequest/taskComplete/sessionDead/answerPlan→auto). `CLAUDE.md` sezione claude riscritta sull'architettura sidecar. Verificato: analyze pulito, 239 test Dart + 14 TS verdi. Deferred: merge MCP-cache da sessionInit (cache privata TTL, serve seam `@visibleForTesting`); MCP auth OAuth (stub); toggle su server remoti claude.ai (provato solo stdio).
>
