      ...
      children: [
        ...
        Expanded(
          child: BlocBuilder<AppLogsCubit, AppLogsState>(
            buildWhen: (p, c) =>
                p.sessions != c.sessions || p.loading != c.loading || p.selectedSessionId != c.selectedSessionId,
...
class LogsDetailView extends HookWidget {
    ...
    useEffect(() {
      void l() {
        ...
        debounce.value = Timer(const Duration(milliseconds: 200), () {
          if (context.mounted) {
            context.read<AppLogDetailCubit>().setSearch(searchController.text);
          }
        });
      }
        ...
        children: [
          Padding(
            ...
            child: Text(
              ...
              style: AppTypography.sidebarLabel.copyWith(color: AppColors.onSurface),
            ),
          ),
          Padding(
          ...
          Expanded(
            child: BlocBuilder<AppLogDetailCubit, AppLogDetailState>(
              buildWhen: (p, c) => p.entries != c.entries || p.loading != c.loading,
              builder: (context, state) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return LogEntryTile(key: ValueKey<int>(e.id), entry: e);
                  },
                );
              },
            ),
  ...
  String _sessionLabel(AppLogSession s) {
    ...
    return s.endedAt == null ? '$start  ·  ${Locales.AppLogs.Session.inProgress}' : start;
  }
      ...
      child: Row(
        children: [
          ...
          Tooltip(
            ...
            child: _IconButton(icon: Symbols.delete_sweep, onTap: () => _confirmDeleteAll(context)),
          ),
        ],
      ),
    );
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.of(ctx).pop(true), child: Text(Locales.AppLogs.deleteAllConfirmOk)),
        ],
      ),
    );
