...
- **claude**: pilota Claude via un **sidecar Node (Agent SDK)** — vedi `backend/` — non più la CLI headless in diretta. La UI Dart parla al sidecar con un **protocollo unico NDJSON su stdio** (`backend/PROTOCOL.md`); il sidecar usa `query()` dell'Agent SDK e la callback `canUseTool`. `SidecarTransport`+`StdioSidecarTransport` spawnano il sidecar; `SidecarClientDataSource` mappa gli eventi del protocollo su `ClaudeEvent` sealed (sessionInit [con mcpServers], textChunk, toolCall/Update/Complete, toolResult, assistantMessage, usageUpdate, taskComplete, errorEvent, rateLimit, sessionDead, permissionRequest, askUserQuestion, **planProposed**). Sessioni one-shot per prompt (resume via session id). History JSONL in `~/.claude/projects/{cwd-encoded}/{sessionid}.jsonl` (list/search/resume/export/delete). **AskUserQuestion e Plan Mode ora interattivi** (round-trip via `canUseTool`): la card piano approva→`auto`/rifiuta→resta plan; le domande tornano al modello con `answers`. Auth OAuth abbonamento (nessuna API key); il transport passa sempre `CLAUDE_CLI_PATH` (via `ClaudeBinaryResolver`) al sidecar.
  - **Permessi**: la logica (read-only allow / mode) è nel sidecar (`decisionFor`); solo il caso "ask" emette `permissionRequest` con UI card (risposta via `respondPermission`). Non c'è più il `PermissionServer` Shelf (rimosso). Modi allineati alla CLI: plan, acceptEdits, auto, bypassPermissions, default.
  - **Controlli**: model (alias sonnet/opus/haiku → ultimo della famiglia), effort (low→max), thinking On/Off (adaptive/disabled), contesto 1M auto dove supportato. MCP: lista pre-caricata (pre-warm all'apertura workspace + refresh gratis da `sessionInit.mcpServers`); toggle via `disabledMcp`→`disallowedTools mcp__<name>__*` (effetto al prompt successivo). MCP auth (needs-auth): TODO.
  - **Packaging (macOS, uso interno)**: `just build-sidecar` bundla il sidecar in un singolo `clyde-sidecar.cjs` (esbuild) embedded in `Clyde.app/Contents/Resources`; in release il transport spawna `node <Resources>/clyde-sidecar.cjs`. Niente SEA/Developer ID/notarization.
